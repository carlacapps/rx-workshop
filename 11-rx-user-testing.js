...
test('Test methods of user module', function (t) {
	...
	var userResult = mockUsers.find(userModule.findUser);
	var todoResult = mockTodos.filter(userModule.filterCompleted);

	t.deepEqual(userResult, mockLeopoldo, 'The `find` logic was able to find Leopoldo!');
	t.deepEqual(todoResult, mockIncompleteTodos, 'The filter was able to remove the completed todos!');
	t.end();
});